...
Now we specify both restriction enzymes using the ``--renz_1```and  ```--renz_2``` flags flags along with the type combinatorial barcoding used. Here is the command:
...
There is additional information available in [http://catchenlab.life.illinois.edu/stacks/comp/process_radtags.php]
